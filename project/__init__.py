...
from .logger import setup_logger  # ← nuevo
    ...
    with app.app_context():
        ...
        if not user_datastore.find_user(email='juan@example.com'):
            user_datastore.create_user(
                ...
                password=encrypted_password, fs_uniquifier=str(uuid4())
            )

        if not user_datastore.find_user(email='admin@example.com'):
            user_datastore.create_user(
                name='Ismael', email='admin@example.com',
                password=encrypted_password, fs_uniquifier=str(uuid4())
            )

        db.session.commit()
        user_datastore.add_role_to_user(
            user_datastore.find_user(email='juan@example.com'), 'end-user')
        user_datastore.add_role_to_user(
            user_datastore.find_user(email='admin@example.com'), 'admin')
        db.session.commit()
